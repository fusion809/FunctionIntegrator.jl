...
@testset "expnx2datan" begin
    @test rectangle_rule(x -> exp.(-x.^2).*(x.^2+1).^(-1), 1e8, -100, 100) ≈ sol_9
    @test simpsons_rule(x -> exp.(-x.^2).*(x.^2+1).^(-1), 1e4, -100, 100) ≈ sol_9
    @test trapezoidal_rule(x -> exp.(-x.^2).*(x.^2+1).^(-1), 1e4, -100, 100) ≈ sol_9
